...
lexer_next :: proc(l: ^Lexer) -> (tk: Token, err: Unmarshal_Error) {
    ...
    current_token := Token{kind = .EOF, location = l.location}
    c := lexer_char(l)
    ...
    switch c {
        ...
        case 'a'..='z', 'A'..='Z': current_token.kind, current_token.data = lex_ident_or_keyword(l)
        case '-':
            if l.in_expression {
        ...
        case '0'..='9': current_token.kind, current_token.data = lex_int_or_float(l) or_return
        case '"': {
            start := l.position
        ...
        case '+': current_token.kind = .Expr_Plus
        case '*': current_token.kind = .Expr_Asterisk
        case '/': current_token.kind = .Expr_SlashForward
        case '(': current_token.kind = .Expr_Lparen
        case ')': current_token.kind = .Expr_Rparen
        case '.': current_token.kind = .Expr_Dot
        case '<':
        ...
        case: return {}, Unexpected_Char{got = c, location = l.location}
    }
    ...
    lex_ident_or_keyword :: proc(l: ^Lexer) -> (Token_Kind, Token_Data) {
        ...
        loop: for {
            switch lexer_peek(l) {
                case 'a'..='z', 'A'..='Z', '0'..='9', '_': lexer_advance(l)
                case: break loop
            }
        }
...
Expression_Node :: union { f64, Host_Variable, Host_Function, Expr_Op }
Host_Variable :: distinct string
Host_Function :: distinct string
Expr_Op :: enum { Add, Subtract, Multiply, Divide }
...
@private
op_tokens: Token_Kind_Set : {
    .Expr_Plus, .Expr_Dash, .Expr_Asterisk, .Expr_SlashForward
}

@private
operator_precedence :: proc(kind: Token_Kind) -> Precedence {
    #partial switch kind {
        case .Expr_Plus, .Expr_Dash: return .Sum
        case .Expr_Asterisk, .Expr_SlashForward: return .Product
        case: return .Lowest
    }
}
...
parse_expression :: proc(p: ^Parser) -> (v: Expression, err: Unmarshal_Error) {
    ...
    parse_primary_expression :: proc(p: ^Parser, prec: Precedence, out: ^[dynamic]Expression_Node) -> (err:Unmarshal_Error) {
        #partial switch p.token.kind {
            case .Lit_Int: append(out, f64(p.token.data.(int)))
            case .Lit_Float: append(out, p.token.data.(f64))
            case .Expr_Lparen: parse_grouped(p, out) or_return
            case .Identifier:
            ...
            case .Expr_Dash: parse_unary_expr(p, out) or_return
            case: return Unexpected_Token{got = p.peek}
        }
    ...
    parse_binary_expr :: proc(p: ^Parser, out: ^[dynamic]Expression_Node) -> (err: Unmarshal_Error) {
        ...
        #partial switch op_token {
            case .Expr_Plus: append(out, Expr_Op.Add)
            case .Expr_Dash: append(out, Expr_Op.Subtract)
            case .Expr_Asterisk: append(out, Expr_Op.Multiply)
            case .Expr_SlashForward: append(out, Expr_Op.Divide)
            case: return Unexpected_Token{got = p.peek}
        }
        return
    }
