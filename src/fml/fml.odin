...
Unexpected_Char :: struct { got: byte, expected: Maybe(byte), location: Location }
Unexpected_Token :: struct { got: Token, expected: Maybe(Token_Kind) }
Type_Error :: struct { got: Token, expected: typeid }
Missing_Key :: struct { key: string, table: string }
Duplicate_Key :: struct { key: string, table: string }
Unknown_Key :: struct { key: string, table: string }
Unsupported_Type :: struct{ tid: typeid }
...
log_error :: proc(err_union: Unmarshal_Error) {
    ...
    switch err in err_union {
	    ...
	    case Type_Error:
	       log.errorf("%s Value provided did not match expected type %v", loc_str(err.got.location), type_info_of(err.expected))
	    case Missing_Key:
	    ...
	    case Unsupported_Type:
	       // by default I want to panic on this, because it's more like a bug then a user error
    ...
    for op_union in expr {
        switch op in op_union {
            ...
            case Expr_Op:
                ...
                a := sa.pop_back(&stack)
                switch op {
                    case .Add: sa.push_back(&stack, a + b)
                    case .Multiply: sa.push_back(&stack, a * b)
                    case .Subtract: sa.push_back(&stack, a - b)
                    case .Divide: sa.push_back(&stack, a / b)
                }
            case Host_Variable:
...
unmarshal_value :: proc(p: ^Parser, v: any, key_name: string) ->  Unmarshal_Error {
    ...
    #partial switch tk.kind {
        case .Lit_Float: if !assign_float(v, tk.data.(f64)) do return type_err
        case .True, .False: if !assign_bool(v, tk.kind == .True) do return type_err
        case  .Lit_String:
            switch &dst in v {
                case string: dst = strings.clone(tk.data.(string)) or_return
                case: return type_err
            }
        case .Lit_Int:
        ...
        case .Lbracket:
            #partial switch inner_info in ti.variant {
                case reflect.Type_Info_Array: unmarshal_array(p, v, inner_info, key_name) or_return
                case reflect.Type_Info_Slice: unmarshal_slice(p, v, inner_info, key_name) or_return
                case: return type_err
            }
    ...
    // basically, if its one variant union just use that
    // if not, try each variant until one doesn't error.
    unmarshal_union :: proc(p: ^Parser, v: any, u: reflect.Type_Info_Union, type_err: Type_Error, key_name: string) -> Unmarshal_Error {
        v := v
        ti := reflect.type_info_base(type_info_of(v.id))
        err: Unmarshal_Error
		for variant, i in u.variants {
			...
			variant_p := p^
			err = unmarshal_value(&variant_p, variant_any, key_name)
			if err == nil {
				p^ = variant_p
				raw_tag := i
    ...
    unmarshal_array :: proc(p: ^Parser, v: any, arr_info: reflect.Type_Info_Array, key_name: string) -> Unmarshal_Error {
    ...
    unmarshal_slice :: proc(p: ^Parser, v: any, sl_info: reflect.Type_Info_Slice, key_name: string) -> Unmarshal_Error {
        backing: []byte
        i: int
        ...
        mem.copy(v.data, &struct{ptr: rawptr, len: int}{raw_data(backing), i}, size_of([]int))
        return nil
    }
}
