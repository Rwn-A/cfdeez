...
vtk_write_vtu :: proc(mesh: cfd.Mesh, vfs: []Out_Vector_Field, sfs: []Out_Scalar_Field, path: string) -> (ok: bool) {
	vtk_w := vtk_writer_init(path) or_return
	defer vtk_writer_destroy(&vtk_w)

	fmt.fprintln(vtk_w.handle, "<?xml version=\"1.0\"?>")

	vtk_open_tag(&vtk_w, .VTKFile, {{"type", "UnstructuredGrid"}, {"version", "0.1"}, {"byte_order", "LittleEndian"}})
	...
	defer vtk_close_tag(&vtk_w)
	{
		...
		vtk_open_tag(
			&vtk_w,
			.DataArray,
			{{"type", "Float64"}, {"Name", "Points"}, {"NumberOfComponents", "3"}, {"format", "binary"}},
		)
	...
	// mesh connectivity
	{
		...
		vtk_open_tag(&vtk_w, .DataArray, {{"type", "Int32"}, {"Name", "connectivity"}, {"format", "binary"}})
		...
		vtk_open_tag(&vtk_w, .DataArray, {{"type", "Int32"}, {"Name", "offsets"}, {"format", "binary"}})
		...
		vtk_open_tag(&vtk_w, .DataArray, {{"type", "Int32"}, {"Name", "types"}, {"format", "binary"}})
		...
		for cell, i in mesh.cells {
			...
			case 3:
				types_data[i] = i32(Type.Tri)
			case 4:
				types_data[i] = i32(Type.Quad)
			case:
				panic("Unsupported element in vtk output, this means the mesh import validation failed.")
			}
		}
	...
	// field values
	{
		...
		for vf in vfs {
			write_vf_binary(&vtk_w, vf.data, vf.name)
		}
		for sf in sfs {
			write_sf_binary(&vtk_w, sf.data, sf.name)
		}
	}

	return true

	write_vf_binary :: proc(vtk_w: ^VTK_Writer, vf: [cfd.VF_COMPONENTS][]f64, name: string) {
		vtk_open_tag(
			vtk_w,
			.DataArray,
			{{"type", "Float64"}, {"Name", name}, {"NumberOfComponents", "3"}, {"format", "binary"}},
		)
		defer vtk_close_tag(vtk_w)

		vector_data := make([]f64, len(vf.x) * 3)
		defer delete(vector_data)

		for i in 0 ..< len(vf.x) {
			vector_data[i * 3 + 0] = vf.x[i]
			vector_data[i * 3 + 1] = vf.y[i]
	...
	write_sf_binary :: proc(vtk_w: ^VTK_Writer, sf: []f64, name: string) {
		vtk_open_tag(vtk_w, .DataArray, {{"type", "Float64"}, {"Name", name}, {"format", "binary"}})
		defer vtk_close_tag(vtk_w)

		vtk_write_binary_data(vtk_w, slice_to_bytes(sf))
	}
}
...
vtk_start_pvd :: proc(directory, sim_name: string) -> (w: VTK_Writer, ok: bool) {
	...
	vtk_open_tag(&w, .VTKFile, {{"type", "Collection"}, {"version", "0.1"}, {"byte_order", "LittleEndian"}})
...
vtk_write_pvd_entry :: proc(vtk_w: ^VTK_Writer, filename: string, time: f64) {
	...
	vtk_open_tag(
		vtk_w,
		.DataSet,
		{{"timestep", strconv.ftoa(strconv_buf[:], time, 'f', 8, 64)}, {"group", ""}, {"part", "0"}, {"file", filename}},
	)
	vtk_close_tag(vtk_w)
}
